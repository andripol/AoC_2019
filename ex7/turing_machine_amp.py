...
def input_to_array(fname, input_value, input_value2):
    global INPUT_VALUE
    with open(fname) as f:
        for line in f:
            for x in line.split(','): program.append(int(x)) 
    INPUT_VALUE = input_value 
    return execute_program(input_value2)
...
def execute_program(input_value2):
    global result
    global INPUT_VALUE
    ip = 0
...
def run(value1, value2):
    result = input_to_array('input7', value1, value2)
    return result
